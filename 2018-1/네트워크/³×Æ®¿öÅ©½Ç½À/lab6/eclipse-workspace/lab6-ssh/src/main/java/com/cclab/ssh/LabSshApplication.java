...
public class LabSshApplication {
    ...
    CommandLineRunner init(RestTemplate restTemplate) {
		...
		if(vc >= 4 && cnt <= 2) {
			...
			String x = "start / min /8 java -jar lab3-0.0.1-SNAPSHOT.jar --server.port=" +(port+cnt);
...
while(true) {	
		...
		Map<String, Integer> metric1 = new HashMap<String, Integer>();
		
// (1) 구현 위치. 다음 문장을 수정함		
		metric1.put("VisitCount", -1);
// (1) end
		
		vc = metric1.get("VisitCount");
		System.out.println("VisitCount: " + vc);	
