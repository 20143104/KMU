...
Lock lock1, lock2, lock3, lock_all;
CondVar c1, c2, c3, c_all;
int num = 0;
...
void take_r1(){
  ...
  while( (Load("resc") == 1 && num == 0) || (Load("r1") == 0) ){
    printf("%d 가 R1을 기다림\n", pid);
    Release(&lock1); Release(&lock_all);
    if(Load("r1") == 0) Wait(&c1, &lock1);
    else Wait(&c_all, &lock_all);
...
void take_r2(){
  ...
  Acquire(&lock_all);
  int s;
  while( (Load("resc") == 1 && num == 0) || (Load("r2") == 0) ){
    printf("%d 가 R2을 기다림\n", pid);
    Release(&lock2); Release(&lock_all);
    if(Load("r2") == 0) Wait(&c2, &lock2);
    else Wait(&c_all, &lock_all);
    Acquire(&lock2); Acquire(&lock_all);
...
void take_r3(){
  ...
  Acquire(&lock_all);
  int s;
  while( (Load("resc") == 1 && num == 0) || ((Load("r3") == 0)) ){
    printf("%d 가 R3을 기다림\n", pid);
    Release(&lock3); Release(&lock_all);
    if(Load("r3") == 0) Wait(&c3, &lock3);
    else Wait(&c_all, &lock_all);
    Acquire(&lock3); Acquire(&lock_all);
