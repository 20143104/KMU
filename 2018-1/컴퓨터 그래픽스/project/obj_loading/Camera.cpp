...
{
  kmuvcl::math::vec4f temp_front_position(front_dir_(0), front_dir_(1), front_dir_(2), 1.0f);
  kmuvcl::math::vec4f temp_up_position(up_dir_(0), up_dir_(1), up_dir_(2), 1.0f);

  kmuvcl::math::mat4x4f rotate = kmuvcl::math::rotate(delta,right_dir_(0),right_dir_(1),right_dir_(2));
  ...
  transformedvector = rotate * temp_up_position;

  up_dir_(0) = transformedvector(0);
  up_dir_(1) = transformedvector(1);
  up_dir_(2) = transformedvector(2);
...
void Camera::unit_vector()
{
  float unit = sqrt(pow(front_dir_(0), 2) + pow(front_dir_(1), 2) + pow(front_dir_(2), 2));

  front_dir_(0) = front_dir_(0) / unit;
  front_dir_(1) = front_dir_(1) / unit;
  front_dir_(2) = front_dir_(2) / unit;

  unit = sqrt(pow(right_dir_(0), 2) + pow(right_dir_(1), 2) + pow(right_dir_(2), 2));

  right_dir_(0) = right_dir_(0) / unit;
  right_dir_(1) = right_dir_(1) / unit;
  right_dir_(2) = right_dir_(2) / unit;

  unit = sqrt(pow(up_dir_(0), 2) + pow(up_dir_(1), 2) + pow(up_dir_(2), 2));

  up_dir_(0) = up_dir_(0) / unit;
  up_dir_(1) = up_dir_(1) / unit;
  up_dir_(2) = up_dir_(2) / unit;
}
